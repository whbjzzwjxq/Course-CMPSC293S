...
class Processor(object):
    def __init__(self, lang: str, code=None, remove_comments=False):
        LANGUAGE = Language("tree_parser/my-languages.so", lang)
        parser = Parser()
        parser.set_language(LANGUAGE)
        self.parser = [parser]
        self.lang = lang
        self.remove_comments = remove_comments
        self.preserve_words = set(
            ["self", "super", "Exception", "__init__", "__main__"]
        )
        if code is None:
            self.tree = None
        else:
    ...
    def load_names(self, path):
        ...
        self.vnames = [
            x
            for (x, _) in self.vnames.most_common(50000)
            if x not in self.preserve_words
        ]
        self.fnames = pickle.load(open(os.path.join(path, "fnames.pkl"), "rb"))
        self.fnames = [
            x
            for (x, _) in self.fnames.most_common(5000)
            if x not in self.preserve_words
        ]
    ...
    def get_func_name(self):
        ...
        root_node = self.tree.root_node
        func_nodes = [
            node for node in root_node.children if node.type == "function_definition"
        ]
        try:
    ...
    def _get_var_names_from_node(self, node, vnames, inactive=False):
        if len(node.children) > 0:
            for child in node.children:
                if (
                    (node.type == "call" and child.type != "argument_list")
                    or (node.type == "attribute")
                    or (node.type in ["import_statement", "import_from_statement"])
                ):
        ...
        if len(node.children) > 0:
            if node.type == "attribute":
                ...
                for child in node.children:
                    if (
                        node.type == "call"
                        and child.type not in ["attribute", "argument_list"]
                    ) or (node.type in ["import_statement", "import_from_statement"]):
                        self._replace_var_names_from_node(child, replaced, True)
                    else:
    ...
    def untokenize(
        ...
        if cut:
            if fix_cut_pos:
                ...
                cut_pos = random.randint(
                    len(self.index_to_code) // 3, len(self.index_to_code) * 2 // 3
                )
    ...
    def convert_to_normal(self, code):
        ...
        for line in lines:
            ...
            res += (
                "\n"
                + " " * indent_size * indent
                + line.replace("<INDENT>", "").replace("<DEDENT>", "")
            )
        return res
